                                ...
                                <v-card color="mainBg" elevation="0">
                                    ...
                                    <div v-show="item.tab === 'Home'" class="pt-7">
                                        <v-row class="mb-5 scroll-vertical-hide v-height" v-if="channelVideos.length > 1">
                                            <v-col class="py-sm-0 py-1" cols="12"
                                                v-for="(item, index) in channelVideos.slice(0, 1)" :key="index">
                                                <VideoCard :video="item.video" :isDetailShow=true />
                                            </v-col>
                                        </v-row>
                                        <v-row v-else-if="showLoader">
                                            <v-col cols="5">
                                        ...
                                        <v-row class="mt-0 ml-0 ml-sm-3 mr-0 mr-xs-2 scroll-vertical-hide v-height"
                                            v-if="channelVideos.length > 1">
                                            <v-col class="py-sm-2 py-1" cols="12" md="3" sm="6"
                                                v-for="(item, index) in channelVideos.slice(1, channelVideos.length - 1)"
                                                :key="index">
                                        ...
                                        <p class="fsn-16 text-center my-16" v-else>This channel does not have any videos.
                                        </p>
                                    </div>
                                    <!-- Videos tab -->
                                    <div v-show="item.tab === 'Videos'" class="pt-4">
                                        <v-row class="mt-0 ml-0 ml-sm-3 mr-0 mr-xs-2 scroll-vertical-hide v-height"
                                            ...
                                            <v-col class="py-sm-0 py-1" cols="12" md="3" sm="6"
                                                v-for="(item, index) in channelVideos" :key="index">
                                                <VideoCard :video="item.video" :isDetail=true />
                                            </v-col>
                                        </v-row>
                                        <v-row v-else-if="showLoader">
                                    ...
                                    <!-- Playlist tab -->
                                    <p class="fsn-16 text-center my-16" v-show="item.tab === 'Playlist'">Sorry, playlist
                                    ...
                                    <div v-show="item.tab === 'Channels'">
                                        <p class="fsn-16 text-center my-16">Sorry, channel details are not available right
                                            now.</p>
                                    </div>
                                    <!-- About -->
...
::v-deep .theme--light.v-tabs .v-tab:focus:before,
